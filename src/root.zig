...
pub const WebView = struct { // const Self = @This();
    webview: quark_webview.webview_t,

    pub const DispatchCallback = fn (view: WebView, data: ?*anyopaque) void;

    pub fn setTitle(self: WebView, title: [:0]const u8) quark_errors.WebViewError!void {
    ...
    pub fn run(self: WebView) quark_errors.WebViewError!void {
        const code = quark_webview.webview_run(self.webview);
        if (code != quark_webview.WEBVIEW_ERROR_OK) return quark_errors.mapError(code);
    }

    pub fn terminate(self: WebView) quark_errors.WebViewError!void {
        const code = quark_webview.webview_terminate(self.webview);
        if (code != quark_webview.WEBVIEW_ERROR_OK) return quark_errors.mapError(code);
    }

    pub fn dispatch(self: WebView, callback: DispatchCallback, data: ?*anyopaque) quark_errors.WebViewError!void {
    ...
    pub fn setSize(self: WebView, width: i32, height: i32, hint: u32) quark_errors.WebViewError!void {
        const code = quark_webview.webview_set_size(self.webview, width, height, hint);
        if (code != quark_webview.WEBVIEW_ERROR_OK) return quark_errors.mapError(code);
    }

    pub fn setHtml(self: WebView, html: [:0]const u8) quark_errors.WebViewError!void {
        const code = quark_webview.webview_set_html(self.webview, html.ptr);
        if (code != quark_webview.WEBVIEW_ERROR_OK) return quark_errors.mapError(code);
    }

    pub fn navigate(self: WebView, url: [:0]const u8) quark_errors.WebViewError!void {
        const code = quark_webview.webview_navigate(self.webview, url.ptr);
        if (code != quark_webview.WEBVIEW_ERROR_OK) return quark_errors.mapError(code);
    }

    pub fn init(self: WebView, js: [:0]const u8) quark_errors.WebViewError!void {
        const code = quark_webview.webview_init(self.webview, js.ptr);
        if (code != quark_webview.WEBVIEW_ERROR_OK) return quark_errors.mapError(code);
    }

    pub fn eval(self: WebView, js: [:0]const u8) quark_errors.WebViewError!void {
        const code = quark_webview.webview_eval(self.webview, js.ptr);
        if (code != quark_webview.WEBVIEW_ERROR_OK) return quark_errors.mapError(code);
    }

    pub fn destroy(self: WebView) quark_errors.WebViewError!void {
